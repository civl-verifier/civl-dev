...
// Command Line Options: /noWitnessInference /noCommutativityTriggers /noVerify /CivlDesugaredFile:desugared_files_async/2pc-triggers.desugared.bpl ../async/2pc-triggers.bpl
...
axiom (forall _0: XState :: 
  { NextStateTrigger(_0): bool } 
  NextStateTrigger(_0): bool <==> true);

function XidTrigger(xid: Xid) : bool;

axiom (forall xid: Xid :: { XidTrigger(xid): bool } XidTrigger(xid): bool <==> true);

function {:inline} ExistsMonotoneExtension(snapshot: GState, state: GState, xid: Xid) : bool
{
  (exists newState: XState :: 
    { NextStateTrigger(newState) } 
...
procedure {:inline 1} GhostRead_8() returns (snapshot: GState);



implementation {:inline 1} GhostRead_8() returns (snapshot: GState)
{
  /*** structured program:
    snapshot := state;
...
procedure {:inline 1} GhostRead_10() returns (snapshot: GState);



implementation {:inline 1} GhostRead_10() returns (snapshot: GState)
{
  /*** structured program:
    snapshot := state;
  **** end structured program */
...
function {:inline} Inv_8(state: GState, B: [Pair]bool, votes: [Xid]int) : bool
{
  gConsistent(state)
     ...
     && (forall xid: Xid :: 
      { XidTrigger(xid) } 
      XidTrigger(xid) ==> VotesEqCoordinatorState(votes, state, xid))
     && (forall xid: Xid :: 
      { XidTrigger(xid) } 
      XidTrigger(xid) ==> votes[xid] == -1 || votes[xid] == card(B, xid))
     && (forall p: Pair :: 
...
procedure Lemma_all_in_B(xid: Xid);
  ...
  ensures card(B, xid) >= numParticipants
     ==> (forall mid: Mid :: 
      { participantMid(mid) } 
      participantMid(mid) ==> B[Pair(xid, mid)]);
...
implementation CommutativityChecker_atomic_AllocateXid_8_atomic_SetParticipantAborted_8(second_xid: Xid, second_mid: Mid, second_pair: Pair)
   returns (first_xid: Xid, first_pairs: [Pair]bool)
{
...
procedure CommutativityChecker_atomic_AllocateXid_8_atomic_SetParticipantAborted_8(second_xid: Xid, second_mid: Mid, second_pair: Pair)
   returns (first_xid: Xid, first_pairs: [Pair]bool);
  requires true;
  requires pair(second_xid, second_mid, second_pair);
  requires xUndecidedOrAborted(state[second_xid]);
...
implementation CommutativityChecker_atomic_AllocateXid_8_atomic_StateUpdateOnVoteYes_8(second_xid: Xid, second_mid: Mid)
   returns (first_xid: Xid, first_pairs: [Pair]bool, second_commit: bool)
{
...
procedure CommutativityChecker_atomic_AllocateXid_8_atomic_StateUpdateOnVoteYes_8(second_xid: Xid, second_mid: Mid)
   returns (first_xid: Xid, first_pairs: [Pair]bool, second_commit: bool);
  requires true;
  requires AllocatedXids[second_xid];
  requires VotesEqCoordinatorState(votes, state, second_xid);
  ...
  ensures (exists partition_pair: [Pair]int :: 
     ...
     ==> (
        AllocatedXids == old(AllocatedXids)[first_xid := true]
         && state == old(state)
         && votes == old(votes)
         && B == old(B)[Pair(second_xid, second_mid) := true]
         && first_xid == first_xid
         && first_pairs
           == (lambda first_p: Pair :: pair(first_xid, mid#Pair(first_p), first_p))
         && (second_commit <==> false)
         ...
         && !old(AllocatedXids)[first_xid]
         && old(votes)[second_xid] == -1)
       || (
        AllocatedXids == old(AllocatedXids)[first_xid := true]
         && state
           == old(state)[second_xid := old(state)[second_xid][CoordinatorMid := (if old(votes)[second_xid := old(votes)[second_xid] + 1][second_xid]
               == numParticipants
             then COMMITTED()
             else old(state)[second_xid][CoordinatorMid])]]
         && votes == old(votes)[second_xid := old(votes)[second_xid] + 1]
         && B == old(B)[Pair(second_xid, second_mid) := true]
         && first_xid == first_xid
         && first_pairs
           == (lambda first_p: Pair :: pair(first_xid, mid#Pair(first_p), first_p))
         && (second_commit
           <==> old(votes)[second_xid := old(votes)[second_xid] + 1][second_xid]
             == numParticipants)
         && old(state)[second_xid := old(state)[second_xid][CoordinatorMid := (if old(votes)[second_xid := old(votes)[second_xid] + 1][second_xid]
               == numParticipants
             then COMMITTED()
             else old(state)[second_xid][CoordinatorMid])]][first_xid]
           == (lambda first_j: Mid :: UNDECIDED())
         && !old(AllocatedXids)[first_xid]
         && old(votes)[second_xid] != -1);
...
implementation GatePreservationChecker_atomic_StateUpdateOnVoteYes_8_atomic_AllocateXid_8(first_xid: Xid, first_mid: Mid)
   returns (first_commit: bool, second_xid: Xid, second_pairs: [Pair]bool)
{
...
procedure GatePreservationChecker_atomic_StateUpdateOnVoteYes_8_atomic_AllocateXid_8(first_xid: Xid, first_mid: Mid)
   returns (first_commit: bool, second_xid: Xid, second_pairs: [Pair]bool);
  requires true;
  requires AllocatedXids[first_xid];
  requires VotesEqCoordinatorState(votes, state, first_xid);
  ...
  ensures true ==> VotesEqCoordinatorState(votes, state, first_xid);



implementation CommutativityChecker_atomic_AllocateXid_8_atomic_StateUpdateOnVoteNo_8(second_xid: Xid, second_mid: Mid)
   returns (first_xid: Xid, first_pairs: [Pair]bool, second_abort: bool)
{
...
procedure CommutativityChecker_atomic_AllocateXid_8_atomic_StateUpdateOnVoteNo_8(second_xid: Xid, second_mid: Mid)
   returns (first_xid: Xid, first_pairs: [Pair]bool, second_abort: bool);
  requires true;
  requires AllocatedXids[second_xid];
  requires !Committed(state[second_xid][CoordinatorMid]);
  ...
  ensures true
     ==> AllocatedXids == old(AllocatedXids)[first_xid := true]
       && state
         == old(state)[second_xid := old(state)[second_xid][CoordinatorMid := (if old(votes)[second_xid] != -1
           then ABORTED()
           else old(state)[second_xid][CoordinatorMid])]]
       && votes == old(votes)[second_xid := -1]
       && first_xid == first_xid
       && first_pairs
         == (lambda first_p: Pair :: pair(first_xid, mid#Pair(first_p), first_p))
       && (second_abort <==> old(votes)[second_xid] != -1)
       && old(state)[second_xid := old(state)[second_xid][CoordinatorMid := (if old(votes)[second_xid] != -1
           then ABORTED()
           else old(state)[second_xid][CoordinatorMid])]][first_xid]
         == (lambda first_j: Mid :: UNDECIDED())
...
implementation GatePreservationChecker_atomic_StateUpdateOnVoteNo_8_atomic_AllocateXid_8(first_xid: Xid, first_mid: Mid)
   returns (first_abort: bool, second_xid: Xid, second_pairs: [Pair]bool)
{
...
procedure GatePreservationChecker_atomic_StateUpdateOnVoteNo_8_atomic_AllocateXid_8(first_xid: Xid, first_mid: Mid)
   returns (first_abort: bool, second_xid: Xid, second_pairs: [Pair]bool);
  requires true;
  requires AllocatedXids[first_xid];
  requires !Committed(state[first_xid][CoordinatorMid]);
...
implementation CommutativityChecker_atomic_AllocateXid_8_atomic_AllocateXid_8()
   returns (first_xid: Xid, 
    first_pairs: [Pair]bool, 
    second_xid: Xid, 
    second_pairs: [Pair]bool)
{
...
procedure CommutativityChecker_atomic_AllocateXid_8_atomic_AllocateXid_8()
   returns (first_xid: Xid, 
    first_pairs: [Pair]bool, 
    second_xid: Xid, 
    second_pairs: [Pair]bool);
  ...
  ensures (exists partition_pair: [Pair]int :: 
      linear_pair_MapImp(PairSetCollector(first_pairs), 
         ...
         && linear_pair_MapImp(PairSetCollector(second_pairs), 
            linear_pair_MapEq(partition_pair, linear_pair_MapConstInt(1)))
           == linear_pair_MapConstBool(true))
     ==> AllocatedXids == old(AllocatedXids)[second_xid := true][first_xid := true]
       && state == old(state)
       && first_xid == first_xid
       && first_pairs
         == (lambda first_p: Pair :: pair(first_xid, mid#Pair(first_p), first_p))
       && second_xid == second_xid
       && second_pairs
         == (lambda second_p: Pair :: pair(second_xid, mid#Pair(second_p), second_p))
       && old(state)[first_xid] == (lambda first_j: Mid :: UNDECIDED())
       && !old(AllocatedXids)[second_xid := true][first_xid]
       && old(state)[second_xid] == (lambda second_j: Mid :: UNDECIDED())
       && !old(AllocatedXids)[second_xid];
...
implementation CommutativityChecker_atomic_AllocateXid_9_atomic_SetParticipantAborted_9(second_xid: Xid, second_mid: Mid, second_pair: Pair)
   returns (first_xid: Xid, first_pairs: [Pair]bool)
{
...
procedure CommutativityChecker_atomic_AllocateXid_9_atomic_SetParticipantAborted_9(second_xid: Xid, second_mid: Mid, second_pair: Pair)
   returns (first_xid: Xid, first_pairs: [Pair]bool);
  requires true;
  requires pair(second_xid, second_mid, second_pair);
  requires xUndecidedOrAborted(state[second_xid]);
...
implementation CommutativityChecker_atomic_AllocateXid_9_atomic_AllocateXid_9()
   returns (first_xid: Xid, 
    first_pairs: [Pair]bool, 
    second_xid: Xid, 
    second_pairs: [Pair]bool)
...
procedure CommutativityChecker_atomic_AllocateXid_9_atomic_AllocateXid_9()
   returns (first_xid: Xid, 
    first_pairs: [Pair]bool, 
    second_xid: Xid, 
    second_pairs: [Pair]bool);
...
implementation CommutativityChecker_atomic_AllocateXid_10_atomic_AllocateXid_10()
   returns (first_xid: Xid, 
    first_pairs: [Pair]bool, 
    second_xid: Xid, 
    second_pairs: [Pair]bool)
...
procedure CommutativityChecker_atomic_AllocateXid_10_atomic_AllocateXid_10()
   returns (first_xid: Xid, 
    first_pairs: [Pair]bool, 
    second_xid: Xid, 
    second_pairs: [Pair]bool);
...
implementation Coordinator_TransactionReq_7() returns (xid: Xid)
{
  ...
  /*** structured program:
    ...
    assert NextStateTrigger(state[xid]);
