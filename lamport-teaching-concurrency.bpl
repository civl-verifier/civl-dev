// Example from "Teaching Concurrency" by Leslie Lamport
...
{
  var i:int;
  assert {:layer 1} !done[0]; // Help solver derive that at least one process is not done
  yield;
...
ensures  {:layer 1} ind_inv(done, y, x);
{
  ...
  call mark_done(i);
  yield;
  assert {:layer 1} x_inv(done, x);
  assert {:layer 1} ind_inv(done, y, x); // Why does this not verify without x_inv(done, x) two lines above?
}
...
function {:inline 1} safety(done: [int]bool, y: [int]int): bool
{
...
function {:inline 1} x_inv(done: [int]bool, x: [int]int): bool
{
...
function {:inline 1} ind_inv(done: [int]bool, y: [int]int, x: [int]int): bool
{
